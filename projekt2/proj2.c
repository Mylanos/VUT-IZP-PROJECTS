 ...
 * @param n [pocet iteracii(presnost)]
 */
 ...
 * @param  arg3 [treti argument]
 * @param  arg4 [stvrty argument]
 * @param  arg5 [piaty argument]
 * @param  x    [ulozene cislo z argumentov z ktoreho pocita Pow]
 * @param  y    [mocnina]
 * @param  n    [pocet iteracii (presnost)]
 * @return      [ERROR(false) - ak nastala chyba, VALID(true) - ak nenastala chyba]
 */
 ...
 * @param  arg3 [treti argument]
 * @param  arg4 [stvrty argument]
 * @param  x    [ulozene cislo z argumentov z ktoreho pocita logaritmus]
 * @param  n    [pocet iteracii (presnost)]
 * @return      [ERROR(false) - ak nastala chyba, VALID(true) - ak nenastala chyba]
 */
 ...
 * @param  x           [logaritmovane cislo]
 * @param  foundResult [TRUE - ak sa nasiel vysledok, FALSE - opak]
 * @return             [cislo (podla definicneho oboru logaritmu)]
 */
 ...
 * @param  x               [umocnovane cislo]
 * @param  y               [mocnina]
 * @param  negativeNumFlag [TRUE - 	ak bolo zadane zaporne cislo, FALSE - opak]
 * @param  foundResult     [TRUE - bol najdeny vysledok]
 * @return                 [cislo (podla definicneho oboru exponencialy)]
 */
...
int main(int argc, char *argv[])
{
	...
	if(argc > 1)			/*kontrola ci bolo zadanych viac argumentov*/
	{
...
void printResultsLog(double x, double n)		//vytlaci vysledky mocniny
{
	printf("       log(%.5g) = %.12g\n", x, log(x));
...
void printResultsPow(double x, double y, double n)		//vytlaci vysledky logaritmu
{	 
...
double cfrac_log(double x, unsigned int n)		//vypocet logaritmu cez zretazene zlomky
{
	...
	result = testLog(x, &foundResult);		//testuje vstupy pre logaritmus
...
double taylor_log(double x, unsigned int n)		//vypocet logaritmu cez taylorove polynomy
{
	...
	{
		if(x < 1)			/*Ak je x mensie ako 1 pouzijeme prvy polynom -> log(1 - y) = -y - (y^2/2) - (y^3/3) - ...*/
		{
		...
		else if(x > 1)			/*Ak je x vacsie ako 1 pouzijeme polynom -> log(x) = ÎŁ(od 1 -> n) (((x - 1) / x)^i) / n*/	
		{
...
double taylor_pow(double x, double y, unsigned int n)		//vypocet mocniny pomocou taylorovho polynomu
{
	...
	unsigned int i = 2;			//inkrementacia zaciname 2 clenom preto i = 2
	double result;				/*vysledok*/
	double lna;					
	double fraction;			/*uklada zlomky*/
...
bool testPowInput(int argc, char *arg3, char *arg4, char *arg5, double *x, double *y, double *n)
{
	...
	{		/*ak bolo zadanych viac ako 5 argumentov program konci, vyhodi chybu*/
...
bool testLogInput(int argc, char *arg3, char *arg4, double *x, double *n)		//kontrola argumentov pre log
{
	...
	{		
		*x = strtod(arg3, &ptr);	/*prevod stringu do double*/
		if(*ptr == 0)
		{				
									/*ak bolo zadane cislo, teda ptr na znak ktory nemoze tvorit double je nulovy
									 *program pokracuje dalej, inak program vyhodi chybu a konci*/
...
{
	if(x < 0)	/*a je x mensie ako 0, vracia NAN pretoze log nie je definoveny pre zaporne cisla*/
	{		
	...
	return 0.0;		/*ak je vsetko OK vratim do result nulu (prvy clen v rozvoji)*/
}
